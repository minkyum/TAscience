...
regCoef <- foreach(i=1:(240*240),.combine='rbind') %dopar% {
  ...
  if(sum(!is.na(sos[i,3:20]))>14 & sum(!is.na(lstAnn[i,]))>14){
    ...
    r1 <- reg$r;  p1 <- reg$regression.results$Slope[3]

    reg <- (lmodel2(sos[i,3:20]~lstSpr[i,]))
    r2 <- reg$r;  p2 <- reg$regression.results$Slope[3]

    reg <- (lmodel2(sos[i,3:20]~lstSum[i,]))
    r3 <- reg$r;  p3 <- reg$regression.results$Slope[3]
...
################
regCoefphe <- foreach(i=1:4,.combine='rbind') %dopar% {
  ...
  if(sum(!is.na(sos[i,1:20]))>14 & sum(!is.na(eos[i,1:20]))>14){
    ...
    c(reg$r,reg$regression.results$Slope[3])
  }else{
    rep(NA,1)
  }
}
colnames(regCoefphe) <- c('r','slope')
print(dim(regCoef))
