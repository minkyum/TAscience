...
url <- paste('https://e4ftl01.cr.usgs.gov/MOTA/MCD43A2.006/',yy,'.',mm1,'.',dd1,'/',sep='')
system(paste('wget --user=twgreen --password=BUremotesensing1! -l1 -r --no-parent -A "MCD43A2.A',yy,'001.',tile,'.006.*.hdf" ',url,sep=''))
url <- paste('https://e4ftl01.cr.usgs.gov/MOTA/MCD43A2.006/',yy,'.',mm2,'.',dd2,'/',sep='')
system(paste('wget --user=twgreen --password=BUremotesensing1! -l1 -r --no-parent -A "MCD43A2.A',yy,doy,'.',tile,'.006.*.hdf" ',url,sep=''))

# for MCD43A2
sstr <- paste('*A',year,'*',tile,'*',sep='')
files <- list.files('//projectnb/modislc/projects/sat/data/e4ftl01.cr.usgs.gov/MOTA/MCD43A2.006',pattern=glob2rx(sstr),recursive=T)
...
while(length(omitted)>0){
  ...
  for(i in 1:length(omitted)){
    ...
    url <- paste('https://e4ftl01.cr.usgs.gov/MOTA/MCD43A2.006/',yy,'.',mm,'.',dd,'/',sep='')
    system(paste('wget --user=twgreen --password=BUremotesensing1! -l1 -r --no-parent -A "MCD43A2.A',yy,doy,'.',tile,'.006.*.hdf" ',url,sep=''))
  }
