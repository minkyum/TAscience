...
lnga2 <- length(list.files('/projectnb/modislc/projects/sat/data/e4ftl01.cr.usgs.gov/MOTA/MCD43A2.006/',pattern=glob2rx(sstr),recursive=T,full.names=T))
lnga4 <- length(list.files('/projectnb/modislc/projects/sat/data/e4ftl01.cr.usgs.gov/MOTA/MCD43A4.006/',pattern=glob2rx(sstr),recursive=T,full.names=T))
...
#  if(year==2000|year==2004|year==2008|year==2012|year==2016|year==2020){
 ...
 # }else{
  ...
  for(i in 1:nday){
    ...
    fld_old <- list.files(paste('/projectnb/modislc/projects/sat/data/e4ftl01.cr.usgs.gov/MOTA/MCD43A4.006/',year,'.',mm,'.',dd,sep=''),pattern=glob2rx(sstr),full.names=T)
    fld_new <- paste('/projectnb/modislc/projects/sat/data/mcd43a4_link/',year,'/',doy,sep='')
    if (!dir.exists(fld_new)) {dir.create(fld_new)}
    system(paste('cp ',fld_old,' ',fld_new,sep=''))    
    
    fld_old <- list.files(paste('/projectnb/modislc/projects/sat/data/e4ftl01.cr.usgs.gov/MOTA/MCD43A2.006/',year,'.',mm,'.',dd,sep=''),pattern=glob2rx(sstr),full.names=T)
